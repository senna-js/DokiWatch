...
export const AnimeStack = (props: AnimeStackProps) => {
  const [animeData, setAnimeData] = useState(null); // State to hold the data
  let accessToken;
  ...
  useEffect(() => {
    if (accessToken) {
      ...
      fetchYourData(accessToken).then((data) => {
        setAnimeData(data); // Update state with the data received
      });
    }
  ...
  query($name: String) {
    ...
    console.log("final data", data.data.MediaListCollection.lists[0].entries);
