...
export const Watchgogo: React.FC = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`https://api-mappings.madara.live/anime/${animeData.alID}`)
            ...
            .then(() => {
                ...
                if (zoroId) {
                    if (zoroId.startsWith("/"))
                        zoroId = zoroId.substring(1);
                    if (zoroId.startsWith("watch/"))
                        zoroId = zoroId.substring(6);
                    if (zoroId.includes("?"))
    ...
    useEffect(() => {
        ...
        const fetchEpisodesData = async () => {
            try {
                await axios.get(`https://consumet-deploy.vercel.app/anime/zoro/info?id=${zoroId}`)
                    .then((response) => {
            ...
            try {
                await axios.get(`https://consumet-deploy.vercel.app/anime/gogoanime/info/${gogoAnimeId}`)
                    .then((response) => {
                        gogoEpisodes = response.data.episodes.map((episode: any) => episode.id);
                    });
            }
            catch (error) {
                console.error("Error with fetching Gogoanime Sub Episodes Data\nGogoAnime Id : ", gogoAnimeId);
            }
            try {
                await axios.get(`https://consumet-deploy.vercel.app/anime/gogoanime/info/${gogoAnimeId}-dub`)
                    .then((response) => {
                        gogoDubEpisodes = response.data.episodes.map((episode: any) => episode.id);
                    });
            }
            catch (error) {
    ...
    useEffect(() => {
        if (!gogoAnimeId || !zoroId || episodesData.length === 0 || currentEpisodeNumber < 0) return;
        var currEpisodeData: currEpisodeData = {
            sources: { sub: { lowRes: "", midRes: "", highRes: "", fullRes: "" }, dub: { lowRes: "", midRes: "", highRes: "", fullRes: "" } }, intro: { start: 0, end: 0 }, outro: { start: 0, end: 0 }
        };
        const fetchCurrEpisodeData = async () => {
            //get current episode intro and outros from zoro
            try {
                await axios.get(`https://consumet-deploy.vercel.app/anime/zoro/watch?episodeId=${episodesData[currentEpisodeNumber - 1].zoroId}`)
                    .then((response) => {
                        console.log(response.data);
                ...
                console.log("Error fetching current episode Intro and Outro\nEpisode Id : ", episodesData[currentEpisodeNumber - 1].zoroId);
            }
            // try {
            await axios.get(`https://consumet-deploy.vercel.app/anime/gogoanime/watch/${episodesData[currentEpisodeNumber - 1].gogoId}`)
                .then((response) => {
                    console.log(response.data);
            ...
            // }
            try {
                await axios.get(`https://consumet-deploy.vercel.app/anime/gogoanime/watch/${episodesData[currentEpisodeNumber - 1].gogoDubId}`)
                    .then((response) => {
                        console.log(response.data);
                ...
                <div className="w-full max-w-4xl relative">
                    {streamUrl ? (
                        <div onLoad={reload}>
                            ...
                            <div className="bg-gray-800 gap-2 border border-white backdrop-blur-lg h-16 flex items-center justify-center px-4 py-auto">
                                ...
                                {(animeData && episodesData[currentEpisodeNumber - 1].gogoDubId) ?
                    ...
                    {settingsVisible && (
                        <div className="flex flex-col absolute top-10 right-2.5 shadow-md z-20 mt-3">
                            {[Quality.lowRes, Quality.midRes, Quality.highRes, Quality.fullRes].map((qualityOption: Quality) => (
                                <div
                                    key={qualityOption}
                                ...
                                >
                                    <div className="opacity-100">{["360p", "480p", "720p", "1080p"][qualityOption]}</div>
                                </div>
                            ))}
                        </div>
                    )}
